  ...
  const onSearch = async value => {
    ...
    getpatient().then(products => {
      setData(products.filter(product => 
        ...
        product.exercises.toLowerCase().includes(value)
        ));
    });
  };
  ...
  const save = async (id) => {
    try {
      ...
      if (index > -1) {
        ...
        item.exercises = row.exercises.split(',');
    ...
    <div className="block contactBlock">
      <div className="container-fluid table">
        <div className="titleHolder">
          ...
          <p>What Exercises will be assign today?</p>
        </div>
        <div>
        <Search placeholder="Diagnosis, Exercises and Name" style={{ width: 380 }} allowClear onSearch={onSearch} enterButton />
